...
class PropertyNumber(models.Model):
    ...
    def action_get_properties(self, tsec, property_use_id_external_id=False, property_building_type_id_external_id=False):
        ...
        #requests
        total_properties = 0
        url = 'https://www.bbva.es/ASO/streetMap/V02/provinces/'+str(self.property_way_id.property_town_id.property_municipality_id.property_state_id.external_id)+'/municipalities/'+str(self.property_way_id.property_town_id.property_municipality_id.external_id)+'/towns/'+str(self.property_way_id.property_town_id.external_id)+'/ways/'+str(self.property_way_id.external_id)+'/numbers/'+str(self.external_id)+'/'
                                                        ...
                                                        for property in number['properties']:
                                                            ...
                                                            if len(property_property_ids)==0:
                                                                ...
                                                                if 'surfaceArea' in property:
                                                                    property_property_vals['surface_area'] = str(property['surfaceArea'])
                                                                #plotSurfaceArea
                                                                if 'plotSurfaceArea' in property:
                                                                    property_property_vals['plot_surface_area'] = int(property['plotSurfaceArea'])
                                                                #builtSurfaceArea
                                                                if 'builtSurfaceArea' in property:
                                                                    property_property_vals['built_surface_area'] = int(property['builtSurfaceArea'])
                                                                #coefficient
                                                                if 'coefficient' in property:
                                                                    property_property_vals['coefficient'] = str(property['coefficient'].replace(',', '.'))
                                                                #yearOld
                                                                if 'yearOld' in property:
                                                                    property_property_vals['year_old'] = int(property['yearOld'])
                                                                #buildingYear
                                                                if 'buildingYear' in property:
                                                                    property_property_vals['building_year'] = int(property['buildingYear'])
                                                                #reformYear
                                                                if 'reformYear' in property:
                                                                    property_property_vals['reform_year'] = int(property['reformYear'])
                                                                #plotRegistryId
                                                                if 'plotRegistryId' in property:
                                                                    property_property_vals['plot_registry_id'] = str(property['plotRegistryId'])
                                                                #property_user_id                                                                
                                                                if 'useCode' in property:
                                                                    if 'id' in property['useCode']:
                                                                        ...
                                                                        if str(property['useCode']['id']) in property_use_id_external_id:
                                                                            property_property_vals['property_user_id'] = property_use_id_external_id[str(property['useCode']['id'])]
                                                                ...
                                                                if 'buildingType' in property:
                                                                    if 'id' in property['buildingType']:
                                                                        ...
                                                                        if str(property['buildingType']['id']) in property_building_type_id_external_id:
                                                                            property_property_vals['property_building_type_id'] = property_building_type_id_external_id[str(property['buildingType']['id'])]                                                                                                                
                                                                #create
                                                                property_property_obj = self.env['property.property'].sudo().create(property_property_vals)
                                                            #total_properties
                                                            total_properties += 1
        else:
    ...
    def cron_check_numbers(self, cr=None, uid=False, context=None):
        ...
        if len(property_way_ids)>0:
            ...
            for property_way_id in property_way_ids:
                ...
                _logger.info(str(property_way_id.name.encode('utf-8'))+' - '+str(percent)+'% ('+str(count)+'/'+str(len(property_way_ids))+')')                                        
                #update
